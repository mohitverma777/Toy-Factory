    ...
    <div class="acc">
        ...
        <div class="ot-box"> 
            <div class="in-boxx width">
                ...
                <input type="text" id="name" name="name" value="<?php echo "$name";?>" required="required" style="width:200px">
                </div>
                <div class="btn-box1">
                    <button type="submit" name="change_name">Save</button>
                    </form>
                </div>
            </div>
        </div>
...
<?php
    ...
    if(isset($_POST['change_name'])){
        $name=$_POST['name'];
        $update_name="UPDATE `usertb` SET `name`='$name' WHERE email='$email'";
        mysqli_query($connection,$update_name);
        echo "<script>alert('name updated');window.location.href ='yourinfo.php'</script>";
    }else{
