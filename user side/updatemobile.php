...
<div id="navbody">
    ...
    <!-- body start -->
    <div class="acc">
        <div class="h-d"><h2>Change your Mobile</h2></div>
        <div class="ot-box"> 
            <div class="in-boxx width">
                
                <div class="inner-box">
                    <strong>New Mobile:</strong><form action="" method="post" style="height:15px; width:400px">
                <input type="text" id="name" name="mobile" minlength="10" maxlength="10" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');" value="<?php echo "$mobile";?>" required="required" style="width:200px">
                </div>
                <div class="btn-box1">
                    <button type="submit" name="change_mobile">Save</button>
                    </form>
                </div>
            </div>
        </div>
...
<?php
    ...
    if(isset($_POST['change_mobile'])){
        $mobile=$_POST['mobile'];
        if (strlen($mobile) < 10) {
            echo "<script>alert('Mobile number must be at least 10 digits long');</script>";
        } else {
            $update_mobile="UPDATE `usertb` SET `mobile`='$mobile' WHERE email='$email'";
            mysqli_query($connection,$update_mobile);
        echo "<script>alert('Mobile updated');window.location.href ='yourinfo.php'</script>";
        }
        
    }else{
        echo"<script>alert(your moblie is not changed)</script>";
    }
?>