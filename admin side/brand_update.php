<?php
  ...
  $brand=$_GET['brand'];
  $select_character="SELECT * FROM brands where brand_id=$brand";
...
<?php
  ...
  move_uploaded_file( $temp_image,"character img/$image_up");
    ...
    $update_qry="UPDATE `brands` SET `brand_title`='$character_title',`brand_image`='$image_up' WHERE brand_id='$chr'";
    
      if(mysqli_query($connection, $update_qry)){
         echo "<script>alert('brand updated');window.location.href ='manage.php';</script>";
      }else{
